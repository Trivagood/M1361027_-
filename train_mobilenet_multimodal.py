...
class MultimodalMobileNetV2(nn.Module):
    def __init__(self, num_classes=4, phys_dim=14):
        ...
        # MobileNetV2 的 classifier 是一個 Sequential，最後一層的輸入維度是 1280
        num_ftrs = self.mobilenet.classifier[1].in_features
        
        # 拔掉原本的分類器，只做特徵萃取
        self.mobilenet.classifier = nn.Identity()
        
        # 建立融合分類器：視覺(1280) + 物理(14) = 1294 維
...
def main():
    ...
    print(f"\n 開始訓練【多模態 MobileNet】，最大 Epochs: {num_epochs} (啟用早停機制) ")
    
    best_val_acc = 0.0
    model_save_path = os.path.join(BASE_DIR, 'multimodal_mobilenet_best.pth') 
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for images, phys_feats, labels in val_loader:
                ...
                loss = criterion(outputs, labels) # 計算測試集的 Loss
        ...
        if val_acc > best_val_acc:
            ...
            print(f" 已儲存目前最佳【多模態 MobileNet】 模型 (準確率: {best_val_acc:.2f}%)")
        else:
            no_improve_count += 1
    ...
    print(f"\n【多模態 MobileNet】最佳準確率為: {best_val_acc:.2f}%")
