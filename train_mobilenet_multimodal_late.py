...
class MultimodalMobileNetV2(nn.Module):
    def __init__(self, num_classes=4, phys_dim=14):
        ...
        # MobileNetV2 的 classifier 最後一層輸入維度是 1280
        num_ftrs = self.mobilenet.classifier[1].in_features
        # 拔掉原本的分類器，讓它只做特徵萃取
        self.mobilenet.classifier = nn.Identity()
        
        # 2. 物理特徵專屬升維網路 (Physics Branch)
        # 物理特徵 14 維 -> 投影至 128 維，對抗 1280 維的視覺特徵霸凌
        ...
        # 3. 融合分類器 (Fusion Classifier)
        # 視覺(1280) + 物理(128) = 1408 維
...
def main():
    ...
    print(f"   [處理前] Class 0 (低量級) 資料量: {class0_original_count} 筆")
    ...
    print(f"   [處理後] Class 0 (低量級) 剩餘: {len(class0_df)} 筆")
    print("=======================================\n")
    ...
    batch_size = 64
    # 防呆：捨棄最後的零頭 batch 確保 BatchNorm 正常運作
    ...
    # 初始化自定義的多模態 MobileNetV2
    model = MultimodalMobileNetV2(num_classes=num_classes, phys_dim=14).to(device)
    ...
    optimizer = optim.Adam([
        {'params': model.mobilenet.parameters(), 'lr': 1e-5},        # 底層輕量網路緩慢微調   
        {'params': model.phys_network.parameters(), 'lr': 1e-3},     # 物理升維網路快速學習
        {'params': model.fusion_classifier.parameters(), 'lr': 1e-3} # 頂層融合網路快速學習
    ])
    ...
    print(f"\n 開始訓練【深度特徵投影多模態 MobileNetV2】，最大 Epochs: {num_epochs} (啟用早停機制)...")
    
    best_val_acc = 0.0
    model_save_path = os.path.join(BASE_DIR, 'multimodal_mobilenet_deep_best.pth') 
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for images, phys_feats, labels in val_loader:
                ...
                outputs = model(images, phys_feats)
                loss = criterion(outputs, labels) # 計算 Val Loss
        ...
        if val_acc > best_val_acc:
            ...
            print(f" 已儲存目前最佳【深度特徵投影多模態 MobileNetV2】(準確率: {best_val_acc:.2f}%)")
        else:
            no_improve_count += 1
    ...
    print(f"\n【深度特徵投影多模態 MobileNetV2】最佳準確率為: {best_val_acc:.2f}%")
    ...
    # 確認下方這兩行轉成 numpy array 的變數名稱是否有對齊：
