...
# 載入第一階段 (CCTV) 訓練好的多模態權重
PRETRAINED_WEIGHTS_PATH = os.path.join(BASE_DIR, 'multimodal_resnet18_deep_best.pth')
# 本次 AIoT 微調後的最終模型
AIOT_SAVE_PATH = os.path.join(BASE_DIR, 'aiot_resnet18_best.pth')
...
def main():
    if not os.path.exists(CSV_PATH) or not os.path.exists(PRETRAINED_WEIGHTS_PATH):
        print(f"找不到 CSV 或預訓練權重檔，請確認路徑")
        return

    df = pd.read_csv(CSV_PATH)
    
    # 確保 AIoT 資料集的 uv_class 也是 4 分類 (0~3)
    df['uv_class'] = df['uv_class'].astype(int).replace(4, 3)
    num_classes = 4
    
    print(f"總共讀取到 {len(df)} 筆 AIoT 多模態訓練資料 ")
    ...
    # 遷移學習：完整載入所有權重
    model = MultimodalResNet18(num_classes=num_classes, phys_dim=14)
    print("啟動遷移學習：載入 CCTV 多模態預訓練模型權重...")
    
    model.load_state_dict(torch.load(PRETRAINED_WEIGHTS_PATH, map_location=device, weights_only=False))
    model = model.to(device)
    print("成功轉移視覺骨幹、物理升維網路與融合分類器的所有權重")

    # 分層差異化學習率
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam([
        {'params': model.resnet.parameters(), 'lr': 1e-5}, 
        {'params': model.phys_network.parameters(), 'lr': 1e-3}, 
    ...
    best_val_acc = 0.0
    print(f"\n【 AIoT 深度特徵投影多模態 ResNet18】開始遷移學習，最大 Epochs: {num_epochs} (啟用早停機制)...")
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            print(f"  已儲存最佳【 AIoT 深度特徵投影多模態 ResNet18】微調權重 (Val Acc: {best_val_acc:.2f}%)")
        else:
            no_improve_count += 1
        ...
        if no_improve_count >= patience:
            ...
            print(f"   模型已連續 {patience} 個 Epoch 未能提升測試準確率，判定已達邊緣端收斂極限。")
            break

    print(f"\n 【 AIoT 深度特徵投影多模態 ResNet18】遷移微調完成，最高測試集準確率為: {best_val_acc:.2f}%\n")
