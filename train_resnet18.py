...
class UVDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            print(f" 警告：無法讀取圖片 {img_name}，將以黑色空圖替代以維持訓練。錯誤: {e}")
...
def main():
    if not os.path.exists(CSV_PATH):
        print(f" 找不到 {CSV_PATH}，請確認資料是否已對齊 ")
        return
    ...
    for i in range(num_classes):
        count = len(df[df['uv_class'] == i])
        if count > 0:
            weights.append(total_samples / (num_classes * count))
        else:
            weights.append(0.0) # 避免除以零

    class_weights_tensor = torch.FloatTensor(weights).to(device)
    print(f" 啟動加權損失函數，各類別權重為: {class_weights_tensor.cpu().numpy()}")
    ...
    print(f"\n 開始訓練 ResNet18，最大 Epochs: {num_epochs} (啟用早停機制) ")
    
    best_val_acc = 0.0
    model_save_path = os.path.join(BASE_DIR, 'resnet18_uv_best.pth') 
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            print(f"  已儲存目前最佳 ResNet18 模型 (準確率: {best_val_acc:.2f}%)")
        else:
            no_improve_count += 1
    ...
    print(f"\n 最佳 ResNet18 模型準確率為: {best_val_acc:.2f}%")
